...
import os
from .. download_manager import (getcwd, cd, mkdir, hide_progress, copy_file)
...
def render_makefile(config, path=None):
    ...
    main_c = os.path.join(path, "main.c")
    mkdir(path)
    if os.path.exists(file_path):
        ...
        with open(makefile_path, "w") as file:
            file.write(content)
        main_path = os.path.join(file_path, "main.c")
        copy_file(main_path, main_c)
        return True
    else:
