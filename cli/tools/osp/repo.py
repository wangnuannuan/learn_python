...
class Repo(object):
    ...
    def fromurl(cls, url, path=None):
        ...
        if m_local:
            ...
            if repo.rev and repo.rev != 'latest' and repo.rev != 'tip' and not re.match(r'^([a-fA-F0-9]{6,40})$', repo.rev):
                print('Invalid revision (%s)' % repo.rev)
        else:
            print('Invalid repository (%s)' % url.strip())
        return repo
    ...
    def clone(self, url, path, rev=None, depth=None, protocol=None, offline=False, **kwargs):
        ...
        if offline:
            print("Unable to clone repository \"%s\" in offline mode ('--offline' used)." % url)
            if os.path.isdir(path):
                rmtree_readonly(path)
    ...
    def write(self): #for lib
        ...
        print("Updating reference \"%s\" -> \"%s\"" % (relpath(cwd_root, self.path) if cwd_root != self.path else self.name, ref))
    ...
    def rm_untracked(self): # untracked some file 
        ...
        for f in untracked:
            if re.match(r'(.+)\.(lib)$', f) and os.path.isfile(f):
                print("Remove untracked library reference \"%s\"" % f)
                os.remove(f)
    ...
    def set_cache(self, url):
        ...
        if cpath and os.path.isdir(self.path):
            try:
                ...
                print("Unable to cache \"%s\" to \"%s\"" % (self.path, cpath))
        return False
