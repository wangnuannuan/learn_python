...
if [ "${TOOLCHAIN}" == "sphinx" ] ; then
    ...
    BUILD_OPTS="OSP_ROOT=${OSP_ROOT} TOOLCHAIN=${TOOLCHAIN} BOARD=${BOARD} BD_VER=${BD_VER} CUR_CORE=${CUR_CORE} TOOLCHAIN_VER=${TOOLCHAIN_VER} EXAMPLES=${EXAMPLES} EXPECTED=${EXPECTED} USER=${AUTO_USER} PASSWORD=${AUTO_PASSWORD} OLEVEL=${OLEVELS}"
    echo ${BUILD_OPTS}
