...
class Gnu(arcToolchain):
    ...
    def check_version():
        ...
        try:
            ...
            if exe is None:
                ...
                self.notifier.event["message"] = "can not execuate {}".format(cmd[0])
    ...
    def download(self, version=None, path=None):
        ...
        if pack_tgz in os.listdir(path):
            self.notifier.event["message"] = "gnu tgz already exists"
            self.notifier.notify(self.notifier.event)
        else:
            self.notifier.event["message"] = "download from (%s)" % url
            self.notifier.notify(self.notifier.event)
            result = download_file(url, gnu_tgz_path)
            if not result:
                self.notifier.event["type"] = "error"
                self.notifier.event["message"] = "download gnu failed"
                self.notifier.notify(self.notifier.event)
                gnu_tgz_path = None
    ...
    def extract_file(self, pack=None, path=None):
        ...
        if pack is None:
            print()
            self.notifier.event["type"] = "warning"
            self.notifier.event["message"] = "please download gnu file first"
            self.notifier.notify(self.notifier.event)
            return False
