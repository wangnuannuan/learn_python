        ...
        if cmd_output:
            ...
            for opt_line in opt_lines:
                if opt_line.startswith("APPL"):
                    build_template["APPL"] = (opt_line.split(":", 1)[1]).strip()
                if opt_line.startswith("BOARD"):
                    build_template["BOARD"] = (opt_line.split(":", 1)[1]).strip()
                if opt_line.startswith("BD_VER"):
                    build_template["BD_VER"] = (opt_line.split(":", 1)[1]).strip()
                if opt_line.startswith("CUR_CORE"):
                    build_template["CUR_CORE"] = (opt_line.split(":", 1)[1]).strip()
                if opt_line.startswith("TOOLCHAIN"):
                    build_template["TOOLCHAIN"] = (opt_line.split(":", 1)[1]).strip()
